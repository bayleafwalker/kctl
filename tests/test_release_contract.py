...
WORKFLOW = ROOT / ".github" / "workflows" / "release-kctl.yaml"
...
def _assert_release_order(workflow: str) -> None:
    ...
    assert workflow.count('"3.12"') == 2  # test matrix plus the build environment
    ...
    assert sync < full_suite < build < validation < attestation < release_create < release_publish
...
def test_release_workflow_contract_rejects_regressions(broken) -> None:
    ...
    with pytest.raises((AssertionError, ValueError)):
        _assert_release_workflow(broken(workflow))
        _assert_release_order(broken(workflow))
...
def test_built_wheel_satisfies_release_contract() -> None:
    ...
    assert len(wheels) == 1
    validate_wheel(wheels[0], tag=f"kctl-v{wheels[0].name.split('-')[1]}")
